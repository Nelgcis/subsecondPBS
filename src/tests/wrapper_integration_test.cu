...
static RTDBeamSettings loadBeamSettingsFromCsv(const std::string& inputDir, const std::string& subspotCsv, const std::string& beamPrefix) {
    ...
    }
    {
        ...
        if (pd0 != static_cast<int>(beam.energies.size()) && rtdVerboseFineTiming()) {
    ...
    if (!hasPositiveSpotSigmas(beam.spotSigmas)) {
        ...
        if (maxSubspots > 0 && rtdVerboseFineTiming()) {
    ...
    if (!perSpotSigmas.empty() && rtdVerboseFineTiming()) {
    ...
    // Remaining wrapper-required values from CSV settings
    {
        ...
        // spotDelta is physical PB spacing, NOT CPB/WEQ spacing. Leave at (0,0,0);
        // the wrapper infers it via buildPhysicalPBLatticeView().
        beam.spotDelta = make_float3(0.0f, 0.0f, 0.0f);
    }
...
static RTDEnergyStruct loadEnergyDataFromCsv(
    ...
    if (iddSliceExport && rtdVerboseFineTiming()) {
    ...
    if (rows != static_cast<int>(energies.size()) && rtdVerboseFineTiming()) {
...
int main(int argc, char** argv) {
    const int verboseLevel = rtdVerboseLevel();
    const bool fineTiming = rtdVerboseFineTiming();
    const bool summaryTiming = rtdVerboseSummary();
    setRTDVerbose(verboseLevel);
    ...
    if (inputMode == FixtureInputMode::Binary) {
        ...
        if (useRef && std::string(useRef) == "1") {
            ...
            if (fineTiming) std::cout << "[TEST] Using reference LUT tables/proton_cumul_ddd_data.txt" << std::endl;
            ...
            if (fineTiming) std::cout << "[TEST] Using synthetic createTestEnergyData() LUT" << std::endl;
        }
    }

    // Create test data
    if (fineTiming) std::cout << "Creating test data..." << std::endl;
    ...
    if (useFixtureInputs && !beam.roiLinearIndices.empty()) {
        ...
        if (fineTiming && useFixtureInputs) {
    ...
    try {
        ...
        if (nonZeroCount > 0) {
            std::cout << "  Average dose (non-zero voxels): " << totalDose / nonZeroCount << std::endl;
        }
        
        // Calculate average dose across all voxels
        float averageDoseAllVoxels = totalDose / doseVolData.size();
        std::cout << "  Average dose (all voxels): " << averageDoseAllVoxels << std::endl;
        ...
        writeDoseOutputs(outputDir, beamPrefix, doseVolDims, doseVolSpacing, doseVolOrigin, doseVolData, useFixtureInputs, fineTiming);
