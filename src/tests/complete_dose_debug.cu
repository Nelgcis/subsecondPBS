...
void testCompleteDoseCalculation() {
    ...
    subsecondWrapper(
        ...
        beamSettings, 1, energyData, 0, false, 1
    );
...
RTDBeamSettings* createTestBeamSettings() {
    ...
    for (int l = 0; l < numLayers; ++l) {
        const float2 sxy = (l < static_cast<int>(beam->spotSigmas.size())) ? beam->spotSigmas[l] : make_float2(2.0f, 2.0f);
        for (int s = 0; s < beam->maxSubspotsPerLayer; ++s) {
            ...
            beam->subspotData[base + 0] = r * cosf(ang);
            beam->subspotData[base + 1] = r * sinf(ang);
            beam->subspotData[base + 2] = 1.0f / beam->maxSubspotsPerLayer;
            beam->subspotData[base + 3] = sxy.x;
            beam->subspotData[base + 4] = sxy.y;
        }
    }

    // Beam geometry (synthetic)
    beam->beamDirection  = make_float3(0.0f, 0.0f, 1.0f);
    beam->bmXDirection   = make_float3(1.0f, 0.0f, 0.0f);
    beam->bmYDirection   = make_float3(0.0f, 1.0f, 0.0f);
    beam->sad            = beam->sourceDist.x;
    beam->sourcePosition = make_float3(0.0f, 0.0f, -beam->sad);
    beam->refPlaneZ      = 0.0f;
    return beam;
}
...
std::vector<float> createTestCTData(int3 dims) {
    ...
    // RayTraceDicom kernels expect HU+1000 in the image volume texture.
    // Create a simple phantom:
    //   - air everywhere (HU=-1000 -> HU+1000=0)
    //   - a central water box (HU=0 -> HU+1000=1000)
    const int cx0 = dims.x / 4;
    const int cx1 = 3 * dims.x / 4;
    const int cy0 = dims.y / 4;
    const int cy1 = 3 * dims.y / 4;
    const int cz0 = dims.z / 4;
    const int cz1 = 3 * dims.z / 4;

    for (int z = 0; z < dims.z; z++) {
        for (int y = 0; y < dims.y; y++) {
            for (int x = 0; x < dims.x; x++) {
                int idx = z * dims.x * dims.y + y * dims.x + x;
                float huPlus1000 = 0.0f; // air
                if (x >= cx0 && x < cx1 && y >= cy0 && y < cy1 && z >= cz0 && z < cz1) {
                    huPlus1000 = 1000.0f; // water
                }
                ctData[idx] = huPlus1000;
            }
        }
    }
