...
EnergyStruct energyReader(const std::string& dataPath) {
    ...
    EnergyStruct eStr;
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] dataPath='" << dataPath << "'" << std::endl;
    std::ifstream fileReader;
    fileReader.exceptions(std::ifstream::failbit);

    const std::string protonFile = resolveTablePath("proton_cumul_ddd_data.txt");
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] protonFile='" << protonFile << "' exists=" << file_exists(protonFile) << std::endl;
    try {
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] opened protonFile" << std::endl;

    fileReader >> eStr.nEnergySamples >> eStr.nEnergies;
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] proton file header nEnergySamples=" << eStr.nEnergySamples
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] densityFile='" << densityFile << "' exists=" << file_exists(densityFile) << std::endl;
    try {
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] opened densityFile" << std::endl;
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] spFile='" << spFile << "' exists=" << file_exists(spFile) << std::endl;
    try {
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] opened spFile" << std::endl;
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] rrlFile='" << rrlFile << "' exists=" << file_exists(rrlFile) << std::endl;
    try {
    ...
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] opened rrlFile" << std::endl;
    ...
    fileReader.close();
    if (rtdVerboseFineTiming()) std::cerr << "[DEBUG][energyReader] finished loading energy tables" << std::endl;
