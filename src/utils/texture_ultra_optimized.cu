...
cudaTextureObject_t create3DTextureUltraOptimized(const float* data, const int3& dims,
    ...
    // [9.45 Device-Backed Texture Contract]
    // Determine correct memcpy kind for the input data pointer.
    // This is critical for correctness: host data must use cudaMemcpyHostToDevice,
    // device data must use cudaMemcpyDeviceToDevice (or cudaMemcpyDefault with UVA).
    // If detection fails, we MUST reject rather than guess HostToDevice for device pointers.
    cudaMemcpyKind copyKind = cudaMemcpyDefault;
    if (data != nullptr) {
        cudaPointerAttributes attrs = {};
        if (cudaPointerGetAttributes(&attrs, data) == cudaSuccess) {
#if CUDART_VERSION >= 10000
            if (attrs.type == cudaMemoryTypeDevice || attrs.type == cudaMemoryTypeManaged) {
                copyKind = cudaMemcpyDefault;  // Use UVA path for device pointers
            } else {
            ...
            cudaGetLastError();
            // If pointer type detection fails, treat as host pointer (conservative).
            copyKind = cudaMemcpyHostToDevice;
        }
    }
