...
// Calculate energyScaleFact using vectorInterpolate
            ...
            if (runtimeNuclearEnabled) {
                ...
                devNucRayWeights = (float*)allocateDeviceMemory(haloWeightsSize);
                copyToDevice(devNucRayWeights, layerHaloPlan->paddedSpotWeights.data(), haloWeightsSize);
                devNucSpotIdx = (int*)allocateDeviceMemory(nucSpotIdxSize);
            ...
            if (runtimeNuclearEnabled) {
                devNucIdd = (float*)allocateDeviceMemory(nucRaySize * sizeof(float));
                devNucRSigmaEff = (float*)allocateDeviceMemory(nucRaySize * sizeof(float));
        ...
        if (rtdSigmaDebugEnabled() && (layerIdx == 0 || layerIdx == (numLayers / 2) || layerIdx + 1 == numLayers)) {
            ...
            // NOTE: already 0-based step indices (RayTraceDicom-main convention).
            ...
            float* devRayIddPadded = nullptr;
            float* devRayRSigmaEffPadded = nullptr;
            ...
            if (superpRayDimsX != rayDims.x || superpRayDimsY != rayDims.y) {
                ...
                const size_t paddedElems = (size_t)superpRayDimsX * (size_t)superpRayDimsY * (size_t)tracerSteps;

                devRayIddPadded = (float*)allocateDeviceMemory(paddedElems * sizeof(float));
                devRayRSigmaEffPadded = (float*)allocateDeviceMemory(paddedElems * sizeof(float));
                ...
                padRayIddSigmaKernel<<<padGrid, padBlock>>>(
                    ...
                    devRayIddPadded, devRayRSigmaEffPadded,
                    rayDims.x, rayDims.y,
                    superpRayDimsX, superpRayDimsY,
                    tracerSteps);
                checkCudaErrors(cudaDeviceSynchronize());

                devRayIddForSuperp = devRayIddPadded;
                devRayRSigmaEffForSuperp = devRayRSigmaEffPadded;
            }
            ...
            float* devBevPrimDose = (float*)allocateDeviceMemory(bevDoseSize);
            ...
            if (runtimeNuclearEnabled) {
                ...
                devBevNucDose = (float*)allocateDeviceMemory(bevNucDoseSize);
            ...
            // Free padded inputs after superposition
            if (devRayIddPadded) freeDeviceMemory(devRayIddPadded);
            if (devRayRSigmaEffPadded) freeDeviceMemory(devRayRSigmaEffPadded);
...
#ifdef NUCLEAR_CORR
            if (devNucIdd) freeDeviceMemory(devNucIdd);
            if (devNucRSigmaEff) freeDeviceMemory(devNucRSigmaEff);
            if (devBevNucDose) freeDeviceMemory(devBevNucDose);
            if (devNucRayWeights) freeDeviceMemory(devNucRayWeights);
            if (devNucSpotIdx) freeDeviceMemory(devNucSpotIdx);
#endif
        }
